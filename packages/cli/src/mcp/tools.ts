...
export function registerTools(server: McpServer, rootDir: string): void {
  ...
  server.tool(
    "list_specs",
    "List all available AgentQA spec files with their scenarios and expectations.",
    {},
    async () => {
      try {
        ...
        return { content: [{ type: "text" as const, text: `Error loading specs: ${err.message}` }], isError: true };
      }
    },
  );
  ...
  server.tool(
    "run_spec",
    "Run a specific AgentQA spec (or a single scenario within it) and return results.",
    {
    ...
    async ({ spec: specName, scenario: scenarioName }) => {
      try {
        ...
        if (matched.length === 0) {
          return { content: [{ type: "text" as const, text: `No spec found matching "${specName}"` }], isError: true };
        }
        ...
        return { content: [{ type: "text" as const, text: `Error running spec: ${err.message}` }], isError: true };
      }
    },
  );
  ...
  server.tool(
    "run_impact",
    "Analyze the current git diff to predict which specs are at risk, then run them.",
    {
  ...
  server.tool(
    "explain_failure",
    "Analyze a failed test scenario and explain the root cause in plain English.",
    {
    ...
    async ({ failure_id }) => {
      try {
        ...
        if (!entry) {
          return { content: [{ type: "text" as const, text: `No failure found matching "${failure_id}"` }], isError: true };
        }
  ...
  server.tool(
    "generate_spec",
    "Auto-generate an AgentQA spec from the current git diff or a specific file.",
    {
      ...
      try {
        ...
        try {
          await generateCommand(target, rootDir, {
            type: type as "web" | "api" | "logic" | undefined,
            dryRun: true,
          });
        } catch {
