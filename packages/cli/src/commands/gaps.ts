...
import ora from "ora";
import { loadAllSpecs, analyzeChangedFiles, AgentQASpec } from "@agentqa/core";
...
export async function gapsCommand(rootDir: string = process.cwd(), options: GapsOptions = {}): Promise<void> {
  ...
  for (const { spec } of analysis.matchedSpecs) {
    if (!spec.trigger.paths) continue;
  ...
  for (const file of changedFiles) {
    ...
    for (const { spec } of specEntries) {
      ...
      if (spec.trigger.paths.some((p: string) => minimatch(file, p))) {
        matches.push(spec.name);
      }
    }
