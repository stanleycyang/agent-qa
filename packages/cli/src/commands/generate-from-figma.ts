 ...
 */
export async function buildFigmaContext(url: string, config: AgentQAConfig): Promise<string> {
  const token = process.env.FIGMA_TOKEN ?? config.integrations?.figma_token;
  if (!token) {
    throw new Error(
      "FIGMA_TOKEN is not set. Get one at https://www.figma.com/developers/api#access-tokens " +
...
function summarizeFigmaNode(node: any, depth: number, maxDepth: number): string {
  if (!node || depth > maxDepth) return "";
  const indent = "  ".repeat(depth);
  const type = node.type ?? "?";
  const name = node.name ?? "";
  let line = `${indent}${type}: ${name}`;
  if (node.characters) line += ` "${String(node.characters).substring(0, 60)}"`;
