...
async function runOnce(
  ...
  const { results: allResults, costInfo, confidenceFloor } = await runSpecsFiltered(
    specEntries, rootDir, options, config,
  );
    ...
    for (const matrixEntry of matrix) {
      ...
      if (result.status !== "pass" && runConfig.autoFix) {
        const fixSpinner = json
          ? { start: () => fixSpinner, succeed: (_m?: string) => {}, fail: (_m?: string) => {} }
          : ora(`     🔧 Investigating fix...`).start();
        if (!json) (fixSpinner as ReturnType<typeof ora>).start();
        try {
          ...
          if (proposal.files.length > 0) {
            ...
            fixSpinner.succeed(
              chalk.cyan(`     🔧 Fix proposed (${(proposal.confidence * 100).toFixed(0)}% confidence, ${proposal.files.length} file(s))${tag}`)
            );
            if (!json) log(chalk.gray(`        ${proposal.summary}`));
          } else {
 ...
 */
async function runSpecsFiltered(
  specEntries: Array<{ spec: AgentQASpec; path: string }>,
  rootDir: string,
  options: RunOptions,
  config: Awaited<ReturnType<typeof loadConfig>>,
): Promise<{ results: SpecResult[]; costInfo: { input_tokens: number; output_tokens: number; usd: number }; confidenceFloor: number }> {
