...
export async function buildSentryContext(
  ...
  if (!token || !org || !project) {
    throw new Error(
      "Sentry integration requires SENTRY_TOKEN, SENTRY_ORG, SENTRY_PROJECT.\n" +
  ...
  for (const issueId of issueIds) {
    try {
      ...
      const breadcrumbs = (event.entries ?? [])
        .find((e: any) => e.type === "breadcrumbs")?.data?.values ?? [];
      const exception = (event.entries ?? [])
        .find((e: any) => e.type === "exception")?.data?.values?.[0];

      const breadcrumbText = breadcrumbs
        .slice(-15)
        .map((b: any) => `  - [${b.category ?? "?"}] ${b.message ?? JSON.stringify(b.data ?? {})}`)
        .join("\n");
