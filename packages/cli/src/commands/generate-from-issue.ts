 ...
 */
export async function buildIssueContext(url: string, config: AgentQAConfig): Promise<string> {
...
async function buildLinearContext(url: string, config: AgentQAConfig): Promise<string> {
  const token = process.env.LINEAR_TOKEN ?? config.integrations?.linear_token;
  if (!token) {
    throw new Error(
      "LINEAR_TOKEN is not set. Get one at https://linear.app/settings/api " +
...
async function buildJiraContext(url: string, config: AgentQAConfig): Promise<string> {
  ...
  if (!token || !host || !email) {
    throw new Error(
      "Jira integration requires JIRA_TOKEN, JIRA_HOST, JIRA_EMAIL.\n" +
