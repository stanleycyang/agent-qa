...
import ora from "ora";
import { loadAllSpecs, analyzeChangedFiles, mergeRankings, RankedSpec } from "@agentqa/core";
import { LogicAgent } from "@agentqa/agents";
import { GitTool } from "@agentqa/tools";
import { loadConfig } from "../config.js";
import { runSpecsFiltered, RunOptions } from "./run.js";
...
export async function impactCommand(
  ...
  let semanticHits: Array<{ name: string; score: number; reason: string }> = [];
  if (process.env.ANTHROPIC_API_KEY) {
    try {
      const specDescriptions = specEntries.map(e => ({
        name: e.spec.name,
        description: e.spec.description ?? "",
        expectations: e.spec.scenarios.flatMap((s: any) => s.expect).slice(0, 5),
      }));
...
Only include specs that are plausibly affected. Return an empty array if none are relevant.`;
      ...
      const result = await (agent as any).runConversation(
        "You are a test impact analyzer. Return only JSON, no tools needed.",
        prompt,
  ...
  const { results, costInfo, confidenceFloor } = await runSpecsFiltered(
    filteredSpecs, rootDir, runOpts, config,
  );
