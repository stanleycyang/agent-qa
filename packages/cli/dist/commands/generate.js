...
export async function generateCommand(target, rootDir = process.cwd(), options = {}) {
    ...
    if (options.dryRun) {
        ...
        for (const spec of specs) {
            console.log(chalk.cyan(`# ${spec.name ?? "untitled"}`));
    ...
    for (const spec of specs) {
        try {
            ...
            if (options.type && typeof spec.environment === "object" && spec.environment) {
                spec.environment.type = options.type;
            }
            const result = await writeSpec(specsDir, spec, { force: options.force });
            written.push(result);
        }
        catch (err) {
            skipped.push({ name: spec.name ?? "untitled", reason: err.message });
        }
    }
    console.log("");
...
async function buildDiffContext(target, options, rootDir) {
