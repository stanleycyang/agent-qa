 ...
 */
export async function executeScenario(spec, scenario, envVars, options) {
    const { agentModel, rootDir, baselineStore, updateBaselines = false, screenshotOnFailure = false, recordVideoOnFailure = false, matrixViewport, matrixBrowser, } = options;
        ...
        try {
            ...
            if (recordVideoOnFailure && replayDir && result && result.status !== "pass") {
                try {
                    ...
                    if (video)
                        result.video_path = path.join(replayDir, video);
                }
                catch {
