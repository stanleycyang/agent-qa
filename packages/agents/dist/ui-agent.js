...
export class UIAgent extends BaseAgent {
     ...
     */
    async clickWithHeal(selector, intent) {
        try {
    ...
    async typeWithHeal(selector, text, intent) {
        try {
    ...
    async healSelector(originalSelector, intent) {
        ...
        try {
            ...
            const result = await this.assertions.assertScreenshot(screenshot.base64, `Looking for an element matching this intent: "${intent}". The original selector "${originalSelector}" did not match anything. Identify the most likely element on the page and describe it precisely (text content, role, position).`);
    ...
    async handleToolCall(name, input) {
        switch (name) {
            ...
            case "click":
                return this.clickWithHeal(input.selector, input.intent);
            case "type":
                return this.typeWithHeal(input.selector, input.text, input.intent);
            case "screenshot":
            ...
            case "check_visual_regression": {
                const screenshot = await this.browser.screenshot();
                const baselineName = input.baseline_name;
                ...
                if (!hasBaseline || this.updateBaselines) {
                    const savedPath = await this.baselineStore.save(this.specName, this.currentScenarioName, baselineName, screenshot.base64);
                    return {
                        pass: true,
                        confidence: 1.0,
            ...
            case "detect_visual_issues": {
                const screenshot = await this.browser.screenshot();
                return this.assertions.detectVisualIssues(screenshot.base64, input.context);
            }
            default:
