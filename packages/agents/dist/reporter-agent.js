export class ReporterAgent {
    generateMarkdown(results, options) {
        ...
        const passedScenarios = results.reduce((sum, r) => sum + r.scenarios.filter(s => s.status === "pass").length, 0);
        const failedScenarios = totalScenarios - passedScenarios;
        const errorScenarios = results.reduce((sum, r) => sum + r.scenarios.filter(s => s.status === "error").length, 0);
            ...
            for (const result of failedResults) {
                for (const scenario of result.scenarios) {
                    ...
                    if (scenario.proposedFix) {
                        ...
                        if (scenario.proposedFix.oversized)
                            failMd += ` ⚠️ oversized — manual review required`;
                        failMd += `\n\n`;
        ...
        if (passedResults.length > 0) {
            ...
            for (const result of passedResults) {
                ...
                for (const scenario of result.scenarios) {
                    ...
                    let tags = "";
                    if (scenario.flaky)
                        tags += ` 🟡 flaky (${(scenario.flaky.rate * 100).toFixed(0)}%)`;
                    if (scenario.perf_regression)
                        tags += ` ⚠️ ${scenario.perf_regression.ratio.toFixed(1)}× slower`;
    ...
    generateJUnit(results) {
        const totalScenarios = results.reduce((sum, r) => sum + r.scenarios.length, 0);
        const failedScenarios = results.reduce((sum, r) => sum + r.scenarios.filter(s => s.status !== "pass").length, 0);
