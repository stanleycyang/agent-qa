...
export class SecurityAgent extends LogicAgent {
  ...
  async handleToolCall(name: string, input: Record<string, unknown>): Promise<unknown> {
    if (name === "scan_secrets") {
      return this.scanForSecrets(input.path as string);
    }
    return super.handleToolCall(name, input);
  }

  private async scanForSecrets(filePath: string): Promise<{ findings: Array<{ pattern: string; line: string; severity: string }> }> {
